...
func (olq *OrderLineQuery) QueryBatch() *BatchQuery {
	query := (&BatchClient{config: olq.config}).Query()
...
func (olq *OrderLineQuery) WithBatch(opts ...func(*BatchQuery)) *OrderLineQuery {
	query := (&BatchClient{config: olq.config}).Query()
...
//		GroupBy(orderline.FieldOrderID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (olq *OrderLineQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*OrderLine, error) {
	...
	if query := olq.withBatch; query != nil {
		if err := olq.loadBatch(ctx, query, nodes, nil,
			func(n *OrderLine, e *Batch) { n.Edges.Batch = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (olq *OrderLineQuery) loadBatch(ctx context.Context, query *BatchQuery, nodes []*OrderLine, init func(*OrderLine), assign func(*OrderLine, *Batch)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*OrderLine)
	for i := range nodes {
		if nodes[i].batch_order_lines == nil {
			continue
		}
		fk := *nodes[i].batch_order_lines
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "batch_order_lines" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
